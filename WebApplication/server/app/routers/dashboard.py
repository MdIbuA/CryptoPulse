...
def _compute_mi_for_coin(settings, coin: str):
    ...
    try:
        ...
        y = df_multi[[f"Close_t+{k}" for k in range(1, 25)]]
...
async def metadata():
    ...
    coins = ["BTCUSDT", "ETHUSDT", "BNBUSDT", "SOLUSDT", "XRPUSDT", "ADAUSDT", "DOGEUSDT", "BCHUSDT", "LTCUSDT"]
    payload = []
    ...
    for coin in coins:
        ...
        if hourly_meta and hourly_meta.exists():
            try:
                ...
                metrics = {
                    "train": {
                        "rmse": metrics_md.get("avg_rmse"),
                        "mae": metrics_md.get("avg_mae"),
                        "r2": metrics_md.get("r2_training"),
                    },
                    "test": {
                        "rmse": metrics_md.get("avg_rmse"),
                        "mae": metrics_md.get("avg_mae"),
                        "r2": metrics_md.get("r2_testing"),
                    },
                }

                # Per-horizon arrays are present in metadata (rmse, mae, r2_by_horizon)
            ...
            if hourly_csv.exists():
                ...
                try:
                    if coin_key:
                        ...
                        model_path = settings.models_hourly_dir / f"{coin_key}_lstm_24h_best.h5"
                        if not model_path.exists():
                            model_path = settings.models_hourly_dir / f"{coin_key}_lstm_24h_model.h5"
...
@router.post("/metadata/{coin}/compute_mi")
async def compute_mi(coin: str):
    """On-demand compute mutual information and feature importance for a coin's hourly dataset.
    This runs the same logic as the internal _compute_mi_for_coin helper but only when explicitly requested by the UI.
    Returns JSON with keys: mi_scores, feature_importance, feature_list.
    """
    coin = coin.upper()
    settings = get_settings()
    ...
    try:
        ...
        # Return empty, don't fail the whole app; the UI can handle empty responses
        return {"coin": coin, "mi_scores": [], "feature_importance": [], "feature_list": []}
